...
- 로그인한 활성 사용자 사이의 학습용 잔액 송금
- 현재 비밀번호 재인증과 CSRF 보호
- 사용자·IP별 송금 횟수 제한과 요청 ID 중복 방지
- 추가 전용 원장과 DB Trigger 기반 잔액 초과 차단
- 송금 당사자만 접근 가능한 페이지 단위 내역
- 관리자 송금 메뉴·처리 경로 차단과 일반 사용자 양방향 송금
- 관리자·사업자 계정을 송금 참여자·수취인에서 제외
...
- [x] 사업자는 판매·판매 주문 조회만 허용하고 송금·구매에서 제외한다.
...
- [x] 받는 사용자를 활성·미탈퇴 사용자 목록에서 서버가 다시 조회한다.
- [x] 본인, 존재하지 않는 사용자, 탈퇴·휴면 사용자를 받는 대상으로 거부한다.
...
- [x] 송금 내역을 보낸 사람과 받는 사람에게만 제공하고 페이지당 20건으로
  제한한다.
...
- 송금 내역이 보낸 사람·받는 사람 외 사용자에게 노출되지 않는지 확인
...
- 사업자가 송금·구매 경로와 수취인 목록에 접근할 수 없고, 판매·판매 주문 조회만 가능한지 확인
- 관리자 계정이 내 상품 관리·상품 등록·수정·삭제, 전체 채팅·1대1 채팅 경로와 Socket 연결을 우회할 수 없는지 확인
