...
모든 테스트가 통과해야 기능·보안·문서 변경을 완료한 것으로 봅니다. 현재 `v6.2`
기준 전체 결과는 `222 passed`입니다. 릴리스별
...
- 관리자 역할 부여·해제와 감사 로그
- 관리자 내 상품 관리·전체 채팅 차단, 역할 간 1대1 채팅, 신고 접수함,
  상품 목록 관리 삭제 버튼·사유·현재 비밀번호 검증
...
임시 DB 권한, 관리자 1명·일반 사용자 2명·사업자 1명 생성, Argon2id 비밀번호
해시, 학습용 송금 잔액, 일반 사용자 명의 샘플 신고, 감사 기록과 외래키
무결성을 확인합니다.
샘플 상품은 사업자 `business_demo`가 판매하는 `사과 한 상자`와
`바나나 한 송이`입니다.
...
상품 테스트는 사업자만 등록·내 상품 관리·수정·일반 삭제를 수행할 수 있는지
확인합니다. 1대1 채팅 테스트는 관리자↔사업자, 일반 사용자↔관리자, 일반
사용자↔사업자 조합과 같은 역할 조합의 HTTP·Socket·DB 차단을 확인합니다.

송금 테스트는 일반 사용자→사업자만 허용하고 일반 사용자끼리의 송금을
수취인 목록·POST·DB Trigger에서 차단하는지 확인합니다. 신고 테스트는 일반
사용자만 접수할 수 있고 사업자 메뉴·경로·DB INSERT와 자기 상품 신고가
차단되는지 확인합니다.
