...
다음 한 줄로 임시 관리자 1명·일반 사용자 2명과 샘플 신고를 만들고 서버를
실행합니다.
...
`user1`, `user2`는 일반 사용자 접근을 확인할 때 사용합니다. 관리자는 송금
기능을 사용할 수 없습니다. `Ctrl+C`로 종료하면
저장소 밖의 임시 DB와 세 계정이 삭제되며 기존 `market.db`는 변경되지 않습니다.
