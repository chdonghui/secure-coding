...
다음 한 줄로 임시 관리자 1명·일반 사용자 2명·사업자 1명과 샘플 상품·신고를
만들고 서버를 실행합니다.
...
`user1`, `user2`는 일반 사용자 접근을, `business_demo`는 사업자 상품 관리와
역할 간 1대1 채팅을 확인할 때 사용합니다. 관리자는 송금·전체 채팅 기능을
사용할 수 없습니다. `Ctrl+C`로 종료하면 저장소 밖의 임시 DB와 네 계정이
삭제되며 기존 `market.db`는 변경되지 않습니다.
...
- 신고 사유와 처리 상태 확인
- 신고 완료 또는 반려 처리
- 상품 목록의 `관리 삭제` 버튼으로 삭제 사유·현재 비밀번호 확인 후 불량 상품
  관리 삭제
- 일반 사용자 휴면·재활성화
- 최근 관리자 처리 감사 이력 조회
- 일반 사용자·사업자와 역할 간 1대1 채팅
