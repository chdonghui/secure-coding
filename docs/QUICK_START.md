...
- 무작위 비밀번호를 사용하는 관리자 `quick_admin`, 일반 사용자 `user1`, `user2`,
  판매 전용 사업자 `business_demo` 생성
- 두 일반 사용자에게 각각 100,000원의 학습용 송금 잔액 생성
- 판매 전용 사업자 `business_demo`가 판매하는 `사과 한 상자`, `바나나 한 송이` 상품 생성
...
2. 상단 `사업자 송금` 메뉴를 누릅니다.
3. 받는 사업자로 `business_demo`를 선택합니다.
4. 금액과 `user1`의 현재 비밀번호를 입력하고 `송금하기`를 누릅니다.
5. 일반 사용자끼리 또는 사업자에서 일반 사용자 방향으로는 송금할 수 없습니다.

상품 구매를 확인하려면 상품 목록에서 `사과 한 상자` 또는 `바나나 한 송이`를
열고 일반 사용자 계정으로 현재 비밀번호를 입력해 구매합니다. 사업자 계정은
상품을 판매하고 상단 `주문 내역`에서 판매 내역을 확인할 수 있습니다. 구매 후
상단 `주문 내역`에서 구매자와
