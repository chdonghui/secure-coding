...
관리, 학습용 상품 구매·주문 내역, 학습용 사용자 간 송금, 전체·1대1 실시간 채팅
및 신고 기능을 포함합니다.
관리자는 별도 권한으로 불량 상품을 관리 삭제하고 불량 사용자를 휴면·해제할 수
있으며, 일반 사용자를 사업자 계정으로 전환하거나 해제할 수 있습니다. 사업자는
상품 판매와 판매 주문 확인만 가능하고 사용자 간 송금·상품 구매에는 참여하지
않습니다.
...
- 상품 거래: 로그인 후 `내 상품 관리`에서 상품을 등록·수정·삭제하고, 일반 사용자는 상품 상세에서 학습용 잔액으로 1회 구매할 수 있습니다.
- 사용자 송금: 일반 사용자(`user1`, `user2`)만 `송금`에서 서로 송금할 수 있습니다. 관리자와 사업자는 송금할 수 없습니다.
- 사업자 계정: 관리자가 `관리자 페이지 → 사용자 관리 → 사업자 권한 부여`를 선택합니다. 사업자는 상품을 등록·판매하고 `주문 내역`에서 판매 내역을 확인할 수 있으며 구매·송금은 제한됩니다. 권한 변경 전 대상 잔액은 0원이어야 합니다.
- 관리자 관리: 관리자만 상품 관리 삭제, 사용자 휴면·복구, 신고 검토, 사업자 권한 부여·해제와 감사 이력 조회를 수행합니다. 관리자 계정에는 내 상품 관리·전체 채팅·1대1 채팅·회원 탈퇴 메뉴가 없으며 해당 경로도 서버에서 차단합니다.
- 실시간 소통·신고: 일반 사용자는 전체 채팅, 1대1 채팅, 사용자·상품 신고를 이용하고, 관리자는 `신고 접수함`에서 접수된 신고를 확인합니다.
...
- 송금은 일반 사용자끼리만 가능하며 관리자·사업자 계정에는 송금 메뉴, 처리
  권한, 송금 대상 자격이 없습니다. `user1`과 `user2`는 서로 양방향으로 송금할
  수 있습니다.
