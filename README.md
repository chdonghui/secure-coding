...
- 회원가입·로그인: `회원가입`에서 계정을 만들고 로그인합니다. 비밀번호는 Argon2id로 저장됩니다.
...
- 관리자 관리: 관리자만 상품 관리 삭제, 사용자 휴면·복구, 신고 검토, 사업자 권한 부여·해제와 감사 이력 조회를 수행합니다. 모든 상태 변경에는 CSRF와 재인증이 필요합니다.
- 실시간 소통·신고: 로그인 후 전체 채팅, 1대1 채팅, 사용자·상품 신고를 이용할 수 있습니다.
