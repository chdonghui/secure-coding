...
로그인, 마이페이지·회원 탈퇴, 공개 상품 조회, 사업자의 상품 등록·수정·삭제와
상품 관리, 학습용 상품 구매·주문 내역, 일반 사용자에서 사업자로 보내는 학습용
송금, 전체·역할 간 1대1 채팅 및 신고 기능을 포함합니다.
관리자는 별도 권한으로 불량 상품을 관리 삭제하고 불량 사용자를 휴면·해제할 수
있으며, 일반 사용자를 사업자 계정으로 전환하거나 해제할 수 있습니다. 사업자는
상품 판매와 판매 주문 확인, 일반 사용자 송금 수취만 가능하며 송금 발신과 상품
구매는 할 수 없습니다.
...
- 회원가입·로그인: `회원가입`에서 중복되지 않은 계정을 만들고 로그인합니다. 가입 계정은 항상 일반 사용자이며 비밀번호는 Argon2id로 저장됩니다. 관리자·사업자 계정은 회원가입으로 만들 수 없습니다.
- 상품 거래: 사업자만 `내 상품 관리`에서 상품을 등록·수정·삭제하고, 일반 사용자는 상품 상세에서 학습용 잔액으로 1회 구매할 수 있습니다.
- 사업자 송금: 일반 사용자는 `사업자 송금`에서 활성 사업자에게만 학습용 잔액을 보낼 수 있습니다. 일반 사용자끼리의 송금과 관리자·사업자의 송금 발신은 차단됩니다.
- 사업자 계정: 관리자가 `관리자 페이지 → 사용자 관리 → 사업자 권한 부여`를 선택합니다. 사업자는 상품을 등록·판매하고 `주문 내역`에서 판매 내역을 확인하며 일반 사용자의 송금을 받을 수 있지만 구매·송금 발신은 제한됩니다. 권한 변경 전 대상 잔액은 0원이어야 합니다.
- 관리자 관리: 관리자만 상품 목록의 `관리 삭제` 버튼으로 삭제 사유·현재 비밀번호를 확인해 상품을 관리 삭제하고, 사용자 휴면·복구, 신고 검토, 사업자 권한 부여·해제와 감사 이력 조회를 수행합니다. 관리자 계정에는 내 상품 관리·전체 채팅·회원 탈퇴 기능이 없습니다.
- 실시간 소통·신고: 전체 채팅은 일반 사용자와 사업자가 이용합니다. 1대1 채팅은 역할이 다른 `관리자↔사업자`, `일반 사용자↔관리자`, `일반 사용자↔사업자` 조합만 허용하며 같은 역할끼리는 대화할 수 없습니다. 신고 접수는 일반 사용자만 가능하고 사업자의 자기 상품 신고는 차단되며, 관리자는 `신고 접수함`에서 신고를 확인합니다.

빠른 실행 스크립트는 관리자, 일반 사용자 2명, 판매 전용 `business_demo` 계정과
사과·바나나 테스트 상품을 임시 DB에 자동 생성합니다. 자세한 계정 정보와 실행
흐름은 [빠른 실행 가이드](docs/QUICK_START.md)를 참고하세요.
...
- 송금 발신은 일반 사용자만 가능하고 수취인은 활성 사업자만 선택할 수 있습니다.
  일반 사용자끼리의 송금과 관리자·사업자의 송금 발신은 화면·서버·DB Trigger에서
  차단됩니다.
- 상품 주문·결제는 외부 결제와 연결되지 않고 학습용 송금 원장만 사용합니다.
  사과·바나나 상품은 테스트 데이터입니다. 구매 시에는 상품 금액이 판매자에게
  학습용 원장으로 이동하고 주문 상태가 `결제 완료`로 기록됩니다.
- 구매·주문은 학습용 내부 잔액만 사용하며 외부 결제창, 카드 승인, 배송, 취소,
  환불, 수수료, 정산, 분쟁 처리는 제공하지 않습니다.
